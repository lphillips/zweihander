...
cat system.template.json | sed -e 's/{{version}}/'"${version}"'/g' -e 's/{{manifest}}/https:\/\/raw.githubusercontent.com\/lphillips\/zweihander\/'"${manifest_branch}"'\/system.json/g' > system.json
